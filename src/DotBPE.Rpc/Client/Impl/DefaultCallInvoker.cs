    ...
    public class DefaultCallInvoker : ICallInvoker<AmpMessage>
        {
            ...
            public async Task<RpcResult<T2>> AsyncCall<T1, T2>(string callName,T1 reqObj, ushort serviceId, ushort messageId, int timeout = 3000)
            {
                RpcResult<T2> result = new RpcResult<T2>();

                var message = AmpMessage.CreateRequestMessage(serviceId, messageId);
                message.FriendlyServiceName = callName;
                try
                {
                    message.Data = this._serializer.Encode(reqObj);
                }
                catch (Exception ex)
                {
                    result.Code = RpcErrorCodes.CODE_INTERNAL_ERROR;
                    _logger.LogError(ex, "message encode error,messageId={0}", message.Id);
                    return result;
                }

                var rsp = await this.AsyncCall(message, timeout);
                ...
                if (rsp != null)
                {
                    ...
                    try
                    {
                        result.Data = this._serializer.Decode<T2>(rsp.Data);
                    }
                    catch (Exception ex)
                    {
                        result.Code = RpcErrorCodes.CODE_INTERNAL_ERROR;
                        _logger.LogError(ex, "message decode code error,messageId={0}", message.Id);
                    }
                    return result;
                }
                else
            ...
            public async Task<RpcResult> AsyncCallWithoutResponse<T>(string callName,T reqObj, ushort serviceId, ushort messageId)
            {
                RpcResult result = new RpcResult();

                var message = AmpMessage.CreateRequestMessage(serviceId, messageId);
                message.InvokeMessageType = InvokeMessageType.InvokeWithoutResponse;
                message.FriendlyServiceName = callName;
                try
                {
                    this._serializer.Encode(reqObj);
                }
                catch (Exception ex)
                {
                    result.Code = RpcErrorCodes.CODE_INTERNAL_ERROR;
                ...
                var rsp = await this.AsyncCall(message, 3000);
                result.Code = rsp.Code;
                return result;
