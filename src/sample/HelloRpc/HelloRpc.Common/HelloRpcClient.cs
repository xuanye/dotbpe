// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: hello_rpc.proto
#region Designer generated code

using System; 
using System.Threading.Tasks; 
using DotBPE.Rpc; 
using DotBPE.Protocol.Amp; 
using DotBPE.Rpc.Exceptions; 
using Google.Protobuf; 

namespace HelloRpc.Common {

//start for class GreeterClient
public sealed class GreeterClient : AmpInvokeClient 
{
public GreeterClient(IRpcClient<AmpMessage> client) : base(client)
{
}
public async Task<HelloResponse> HelloPlusAsnyc(HelloRequest request,int timeOut=3000)
{
AmpMessage message = AmpMessage.CreateRequestMessage(100, 1);
message.Data = request.ToByteArray();
var response = await base.CallInvoker.AsyncCall(message,timeOut);
if (response != null && response.Data !=null)
{
return HelloResponse.Parser.ParseFrom(response.Data);
}
throw new RpcException("请求出错，请检查!");
}

//同步方法
public HelloResponse HelloPlus(HelloRequest request)
{
AmpMessage message = AmpMessage.CreateRequestMessage(100, 1);
message.Data = request.ToByteArray();
var response =  base.CallInvoker.BlockingCall(message);
if (response != null && response.Data !=null)
{
return HelloResponse.Parser.ParseFrom(response.Data);
}
throw new RpcException("请求出错，请检查!");
}
}
//end for class GreeterClient

//start for class MathClient
public sealed class MathClient : AmpInvokeClient 
{
public MathClient(IRpcClient<AmpMessage> client) : base(client)
{
}
public async Task<AddResponse> AddAsnyc(AddRequest request,int timeOut=3000)
{
AmpMessage message = AmpMessage.CreateRequestMessage(101, 1);
message.Data = request.ToByteArray();
var response = await base.CallInvoker.AsyncCall(message,timeOut);
if (response != null && response.Data !=null)
{
return AddResponse.Parser.ParseFrom(response.Data);
}
throw new RpcException("请求出错，请检查!");
}

//同步方法
public AddResponse Add(AddRequest request)
{
AmpMessage message = AmpMessage.CreateRequestMessage(101, 1);
message.Data = request.ToByteArray();
var response =  base.CallInvoker.BlockingCall(message);
if (response != null && response.Data !=null)
{
return AddResponse.Parser.ParseFrom(response.Data);
}
throw new RpcException("请求出错，请检查!");
}
}
//end for class MathClient
}
#endregion
