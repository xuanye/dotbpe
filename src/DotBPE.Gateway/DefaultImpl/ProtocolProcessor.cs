    ...
    public class ProtocolProcessor:IProtocolProcessor
    {
        ...
        private async Task<bool> InvokeInner(HttpContext context)
        {
            ...
            catch (Exception ex)
            {
                ...
                this._logger.LogError(ex, "call error:{url}\r\n:------------\r\ndata:{data}\r\n--------------------------"
                    ,req.Path,Newtonsoft.Json.JsonConvert.SerializeObject(requestData));
                hasRes = true;
            }
        ...
        private async Task<bool> ParseRequestAsync(HttpRequest req, HttpResponse res,HttpRequestData requestData,RouteItem router)
        {
            ...
            try
            {
                await ProcessRequestData(req, router, requestData);
                CollectCommonData(req, requestData.QueryOrFormData);
            }
            catch (Exception ex)
            {
        ...
        protected virtual async Task ProcessRequestData(HttpRequest req, RouteItem router, HttpRequestData requestData)
        {
            ...
            if (contentType == "application/x-www-form-urlencoded" || contentType == "multipart/form-data")
                ...
                requestData.RawBody = await CollectBody(req.Body);
        }
